...
extension ViewController {
    ...
    @IBAction func changePlayerControlSegment(_ sender: UISegmentedControl) {
        ...
        if !isAnimating, side == gameState.turn, case .computer = Player(rawValue: sender.selectedSegmentIndex)! {
            playTurnOfComputer()
        }
    }
}
...
extension ViewController {
    ...
    func saveGame() throws {
        let players = playerControls.map { $0.selectedSegmentIndex.description }
        try gameState.saveGame(at: path, players: players)
    }
    
    /// ゲームの状態をファイルから読み込み、復元します。
    func loadGame() throws {
        let players = try gameState.loadGame(from: path)
        for (i, player) in players.enumerated() {
            playerControls[i].selectedSegmentIndex = player.rawValue
        }
